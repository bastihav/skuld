...
public class ManagementApiController implements ManagementApi {
  ...
  public ResponseEntity<MetaData> createTree(
    ...
    if (accept != null && (accept.contains("application/json") || accept.contains("*/*"))) {
      UUID uuid = radixManager.createNewDiskBasedRadixTrie(
          Date.from(Instant.parse(body.getDate())));
  ...
  public ResponseEntity<InlineResponse200> getSetUpdater(
    ...
    if (accept != null && (accept.contains("application/json") || accept.contains("*/*"))) {
      if (setRunning != null) {
        ...
        if (!setRunning) {
          radixManager.stopUpdaterThread();
        }
      }
  ...
  public ResponseEntity<List<MetaData>> getTrees() {
    String accept = request.getHeader("Accept");
    if (accept != null && (accept.contains("application/json") || accept.contains("*/*"))) {
      List<MetaData> metaDataList = radixManager.getTries().values().stream()
          .map(trie -> trie.getMetaData()).map(RadixMetaData::toAPIMetaData).collect(
              Collectors.toList());
