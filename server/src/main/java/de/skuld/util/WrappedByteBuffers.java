...
public class WrappedByteBuffers {
  ...
  /**
   * Gets the element described by index, i.e. the element with index 0 is the first element, aka
   * first 32 bytes of the first mbb. The element with index 1 is the second element, aka [32..63]
   * of the first mbb
   * <p>
  ...
  public byte[] get(int index) {
    ...
    dataArray[getIndexOfMBB(index)].duplicate().position(getOffsetInMBB(index) * chunkSize)
        .get(result);
  ...
  public void sort() {
    ...
    forkJoinPool.invoke(
        new ParallelMergeSort(this, indexArray, helper, firstBytes, 0, elementCount - 1));
  }
